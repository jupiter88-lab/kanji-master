  ...
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        // 設問数が多い練習プリント等で JSON が途中で切れないよう、十分大きく取る
        max_tokens: 8000,
        system: systemPrompt,
        messages: [
          {
            ...
            content: [
              ...
              {
                type: 'text',
                text: 'このテスト用紙を採点してください。必ず指定のJSON形式のみで、コードフェンスや前置きを付けずに返答してください。'
              }
            ]
          },
          {
            // アシスタントを "{" で prefill。これで Claude は必ず JSON 本体から書き始める
            // （フロント側で先頭に "{" を付け直してパースする）
            role: 'assistant',
            content: '{'
          }
        ]
